...
def test_replay_writes_required_artifacts_and_fields():
    ...
    for key in ["run_id", "ts", "token_address", "pair_address", "symbol", "config_hash", "decision", "entry_confidence", "recommended_position_pct", "final_score_pre_wallet", "discovery_lag_penalty_applied", "discovery_lag_blocked_trend", "discovery_lag_size_multiplier", "discovery_lag_score_penalty", "tx_batch_status", "tx_batch_freshness", "tx_fetch_mode", "tx_batch_warning", "wallet_weighting_requested_mode", "wallet_weighting_effective_mode", "wallet_score_component_raw", "wallet_score_component_applied", "wallet_score_component_applied_shadow", "replay_score_source", "wallet_mode_parity_status", "score_contract_version", "historical_input_hash", "replay_input_origin", "replay_data_status", "replay_resolution_status", "schema_version"]:
        assert key in matrix_row
    assert matrix_row["net_pnl_pct"] < matrix_row["gross_pnl_pct"]
