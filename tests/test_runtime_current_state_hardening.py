...
def test_open_position_prefers_market_states_artifact_for_live_refresh(tmp_path):
    processed = tmp_path / "processed"
    config_path = _config(tmp_path)
    run_id = "runtime_market_states_refresh"
    ...
    write_json(
        processed / "entry_candidates.json",
        {
            "tokens": [
                {
                    "signal_id": "open_market_state_1",
                    "token_address": "SoMarket111",
                    "pair_address": "PairMarket111",
                    "entry_decision": "SCALP",
                    "regime": "SCALP",
                    "x_status": "healthy",
                    "signal_ts": "2026-03-20T00:00:00+00:00",
                    "recommended_position_pct": 0.4,
                    "entry_snapshot": {"price_usd": 1.0, "liquidity_usd": 25000, "buy_pressure": 0.81, "volume_velocity": 4.1, "x_validation_score": 81, "x_status": "healthy"},
                }
            ]
        },
    )
            ...
            "market_states": [
                {
                    ...
                    "price_usd_now": 0.91,
                    "liquidity_usd": 27000,
                    "liquidity_usd_now": 27000,
                    "buy_pressure": 0.77,
                    "buy_pressure_now": 0.77,
                    "volume_velocity": 3.8,
                    "volume_velocity_now": 3.8,
                    "x_validation_score": 78,
                    "x_validation_score_now": 78,
                    "x_status": "healthy",
    ...
    summary = read_json(tmp_path / "runs" / run_id / "daily_summary.json", default={})
    positions = read_json(tmp_path / "runs" / run_id / "positions.json", default={})

    assert summary["runtime_current_state_live_count"] >= 1
    assert summary["runtime_current_state_fallback_count"] == 0
    assert positions["open_positions"][0]["last_mark_price_usd"] == 0.91
